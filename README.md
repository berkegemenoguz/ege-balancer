...
after the time it was given, at most ten minutes. `GET /metrics` on the same port is what Prometheus
reads: what the backend did with each request, the faults in force, its workers, queue, cold start
and cache.
...
balancer and the mock backends every five seconds, and Grafana on [localhost:3000](http://localhost:3000)
with four linked dashboards in an *Ege-Balancer* folder:
...
Every graph marks configuration reloads, so the moment an algorithm changes is visible, and the time
a fault injected into a mock backend was in force. Each backend keeps its profile's colour, and a
window selector trades detail for smoothness. The dashboards are generated by
`deploy/grafana/generate_dashboards.py`; edit the script and run it, rather than editing the JSON.
