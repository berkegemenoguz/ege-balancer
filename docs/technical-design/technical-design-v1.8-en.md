...
| `lb_backend_failures_total` | counter | backend, reason | attempts a backend could not serve, and why |
...
A failed attempt carries one of six reasons, in the order of the exchange: `connect` when no
connection could be made, `timeout` when no answer began within the response timeout, `reset` when
the connection closed before the answer began, `cut_off` when the answer was broken off part way,
`5xx` for a 5xx answer under `retry_on_5xx`, and `other` for anything else. A client that gives up is
not counted at all (§6.3). Each backend's six counters exist from the start, at zero: a series that
first appears with its first failures starts at their count, and `rate()` cannot see a jump from
nothing, so a backend that went down once would show no failures. The reason was added after v1.3.2
(Appendix B, entry 17).
...
The Compose environment runs Prometheus, scraping the balancer and the admin port of each mock
backend (§9.4) every five seconds, and Grafana with four provisioned, linked dashboards. *Overview*
is the one to keep on screen: headline figures, request rate and share of traffic per backend,
latency and a health timeline. *Backends* compares the backends — a table, share of traffic,
requests in flight averaged over a window, p95 latency per backend and a latency heatmap.
*Resilience* shows what failures do: answers by status class, refusals by reason, retries against
the budget and reloads. *Faults* follows a fault from the backend to the client: the faults
injected; what the mock backends did instead of answering, with their workers, queues, cold starts
and caches; the balancer's failed attempts by reason and backend, its health checks and retries; and
what clients received, answers cut off part way included. A mock's series are labelled with the
address the balancer forwards to, so the two views of one backend line up.

Three choices make them readable. Every graph marks configuration reloads and the time a fault
injected into a mock backend was in force, so an effect is seen against the moment that caused it.
Each backend keeps the colour of its profile (§9.4). And the in-flight gauge is shown averaged:
sampled every five seconds at light load it reads mostly 0 or 1, which says nothing about how busy a
backend is. The dashboards are generated by a script, so the four stay consistent. Two resource findings from
...
| `admin` | a second port through which faults are started and cleared while the backend runs, and which serves its metrics |
...
demo console's fault action uses it. It also serves the backend's own metrics in the Prometheus
format — what the backend did with each request, the faults in force, its workers, queue, cold start
and cache — written with the standard library, so that the mock stays free of dependencies (§8.3).
...
### B.17 Failed attempts carry a reason, and the mock backends are scraped too

*v1.6 §7.6 · after v1.3.2 · §8.1, §8.3, §9.4*

**v1.6.** Prometheus scrapes the load balancer's `/metrics`, and one Grafana dashboard shows the
request rate, the error rate, latency, active connections and health.

**Instead.** `lb_backend_failures_total` carries a `reason` — connect, timeout, reset, cut_off, 5xx
or other. Prometheus also scrapes the admin port of each mock backend, which reports what the
backend did with its requests and which faults are in force. A fourth dashboard, *Faults*, puts the
two side by side, and every dashboard marks the time a fault was in force.

**Why.** Once the mock backends could fail in several ways (entry 13), a count of failures could no
longer say which: a hang, a dropped connection and a stopped container raised the same counter. From
the balancer alone a fault can be inferred but not confirmed; the backend's own count shows the
cause, and the balancer's the effect. The mocks' metrics are written with the standard library
